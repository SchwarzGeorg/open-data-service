...
DEFAULT_USERSERVICE_URL="http://localhost:8090/ods/userservice/api/v1"
...
if [[ ! -f finished-ods-config ]]; then
	...
	if [[ -f $CONFIG_FILE ]]; then
		...
		sed -ie "s/USERSERVICE_URL/${DEFAULT_USERSERVICE_URL:-DEFAULT_USERSERVICE_URL}/" $CONFIG_FILE
	fi
	cat $CONFIG_FILE
